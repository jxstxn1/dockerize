...
Future<void> createServerFolder(SidekickPackage package) async {
  final serverFolder = findRepository().root.directory('server').createIfNotExists();
  ...
  serverFolder.file('bin/server.dart').writeAsStringSync(
        replaceTemplateDependencies(
          PluginContext.installerPlugin.root.file('template/server.template.dart').readAsLinesSync(),
          package.cliName,
        ).join('\n'),
      );

  serverFolder.file('bin/middlewares.dart').writeAsStringSync(
        PluginContext.installerPlugin.root.file('template/middlewares.template.dart').readAsStringSync(),
      );

  serverFolder.file('Dockerfile').writeAsStringSync(
        PluginContext.installerPlugin.root.file('template/Dockerfile.template').readAsStringSync(),
      );
  serverFolder.file('pubspec.yaml').writeAsStringSync(
        PluginContext.installerPlugin.root.file('template/pubspec.template.yaml').readAsStringSync(),
      );
  serverFolder.file('analysis_options.yaml').writeAsStringSync(
        PluginContext.installerPlugin.root.file('template/analysis_options.template.yaml').readAsStringSync(),
      );
  serverFolder.file('README.md').writeAsStringSync(
        PluginContext.installerPlugin.root.file('template/README.template.md').readAsStringSync(),
      );
