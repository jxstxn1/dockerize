...
Future<void> main() async {
  ...
  pubGet(package);
  final commandFolder = package.root.directory('lib/src/commands/dockerize').createIfNotExists();
  ...
  dockerCommandFile.writeAsStringSync(
    replaceTemplateDependencies(
      PluginContext.installerPlugin.root.file('template/docker_command.template.dart').readAsLinesSync(),
      package.cliName,
    ).join('\n'),
  );

  final environmentFile = commandFolder.file('environment.dart');
  PluginContext.installerPlugin.root.file('template/environment.template.dart').copySync(environmentFile.path);

  final buildCommandFile = commandFolder.file('build_command.dart');
  buildCommandFile.writeAsStringSync(
    replaceTemplateDependencies(
      PluginContext.installerPlugin.root.file('template/build_command.template.dart').readAsLinesSync(),
      package.cliName,
    ).join('\n'),
  );
  ...
  runCommandFile.writeAsStringSync(
    replaceTemplateDependencies(
      PluginContext.installerPlugin.root.file('template/run_command.template.dart').readAsLinesSync(),
      package.cliName,
    ).join('\n'),
  );

  final stopCommandFile = commandFolder.file('stop_command.dart');
  PluginContext.installerPlugin.root.file('template/stop_command.template.dart').copySync(stopCommandFile.path);

  await registerPlugin(
    sidekickCli: package,
    import: "import 'package:${package.name}/src/commands/dockerize/docker_command.dart';",
